...
dat = predicsis.Dataset.create(name='My dataset',file='C:/Users/PC/Desktop/Clothing_Store.txt',header=True,separator=',')
print dat.name
...
target = predicsis.Target.create(target_var="RESP",dictionary_id=dico.id)
...
scoresets = predicsis.Scoreset.create(name="My scores",dictionary_id = dico.id, model_id = model.id, data = 'C:/Users/PC/Desktop/Clothing_Store.txt', header=True,separator=',', file_name='out.txt', papa='papa')
...
print r2.id
r3 = predicsis.Report.create(type = "classifier_evaluation", dataset_id = dat.id, dictionary_id = dico.id, model_id = model.id, main_modality="1")
print r3.id
